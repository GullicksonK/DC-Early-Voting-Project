...
if (!Getopt::Long::GetOptions
                ("v|verbose+"   => \$verbose,
                 ...
                 "p|precinctMap=s" => \$precintMapFile,
                 "e|election=s" => \$election,
                 "k|kmlFile=s" => \$kmlFile,
                 "nop" => \$dry_run,
                 "<>" => \&addFileOption))
{
...
open(PMFD,"$precinctMapFile") || die "$whoami: Unable to open $precinctMapFile: $!\n";
chomp(@precinctMapData = <PMFD>);
close(PMFD);

foreach $line (@precinctMapData)
{
   ($precinct, $longitude, $latitude, $polyIdx) = split(/,/,$line);
}
